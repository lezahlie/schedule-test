...
for line in args.inputSWF:
    ...
    if res:
        ...
        print(f'LOOK >>> swf_to_batsim_output_csv.py || job_id = {job_id} || walltime = {wall_time} = MAX[ run_time[{run_time}], requested_time={float(res.group(SwfField.REQUESTED_TIME.value))} ] ')
        if(nb_res > 0 and

...
for line in args.inputSWF:
    ...
    if res:
        ...
        print(f'LOOK >>> swf_to_batsim_workload_compute_only.py || job_id = {job_id} || walltime = {walltime} = MAX[ (job_walltime_factor[{args.jobWalltimeFactor}] * run_time{run_time}), requested_time={float(res.group(SwfField.REQUESTED_TIME.value))} ] ')

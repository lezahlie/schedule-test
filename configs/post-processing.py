...
df3=df3[cols]
print(f"df3[t_exe] = {df3['total_execution_time']}")
if checkpointing:
...
def get_makespan_df(ourDf,ourDf3,total_makespan,checkpointing):
    ...
    if reservations_as_jobs:
        ...
        avg_exe = ourDf3['total_execution_time'].mean()
        if pp_slowdown:
    ...
    sec4=timedelta(seconds=(int(avg_exe)))
    avg_exe_dhms = str(sec4)
    avg_waiting_dhms = str(sec3)
    avg_tat_dhms =str(sec2)
    makespan_dhms = str(sec)
    
    # @note mke span csv df
    makespan_df = pd.DataFrame({"nodes":numNodes,
                                ...
                                "AAE":[avgAE],
                                "avg_execution":[avg_exe],
                                "avg_execution_dhms":[avg_exe_dhms],
                                "avg_tat":[avg_tat],

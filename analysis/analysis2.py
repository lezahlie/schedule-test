...
if not (sys.argv[3] in [ "--all", "--outliers", "--non-outliers" , "--comparisons" ] ):
    ...
    folder1=sys.argv[2]
    # @note LH wl db change
    # database = pd.read_csv(folder1+"/grizzly_workloads_db.csv",header=0,sep="|")
    database = pd.read_csv(folder1+"/synth_workloads_db.csv",header=0,sep="|")
    ...
    def plotMyGraphs(cutOutliersTF=True,pp_slowdown=60,data_also=False,data_only=False,runs=[1,1]):
        ...
        for index, row in database.iterrows():
            ...
            binned_means = df.groupby('binned', dropna=True).mean(numeric_only=True)[metric].values
            binned_maxs = df.groupby('binned').max()[metric].values
            binned_mins = df.groupby('binned').min()[metric].values
            binned_stds = df.groupby('binned').std(numeric_only=True)[metric].values
    ...
    if graph_option == "--all":
        ...
        # @note LH changed # runs
        plotMyGraphs(cutOutliersTF=True,runs=[1,3],data_only=True)
    ...
    # @note LH pring binned means
    print(f'database["binned-means"] = {database["binned-means"]}', end="")
    ...
    for i in database.time.unique():
        ...
        plt.yticks(ticks=[tick for tick in np.arange(0,102+.5,2.5)],labels=[f"{tick}%" for tick in np.arange(0,102+.5,2.5)])
        plt.ylim(0,102)
        count=0
        for j in myList:
    ...
    for i in database["subdivisions-unit"].unique():
        ...
        for k in df["time"].unique():
            ...
            plt.yticks(ticks=[tick for tick in np.arange(0,105,5)],labels=[f"{tick}%" for tick in np.arange(0,105,5)])
            ...
            for j in range(0,11,1):
                ...
                print(f'y ={y}')
                # @note LH fixed nan values
                if np.isnan(y[0]):
                    y[0] = np.nan_to_num(y[0])
                if y[0] > 0:

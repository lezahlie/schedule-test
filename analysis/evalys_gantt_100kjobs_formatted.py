...
class GanttVisualization(core.Visualization):
    ...
    def __init__(self, lspec, *, title='Gantt chart', labeler = True):
    ...
    def build(self, jobset):
        ...
        self._ax.set(
            xlim=(df.submission_time.min(), df.submission_time.max()),
            ylim=(jobset.res_bounds.inf, jobset.res_bounds.sup + 2),
        )
...
def plot_gantt(jobset, *, title='Gantt chart', labeler = True, **kwargs):
    """
    ...
    plot = layout.inject(GanttVisualization, spskey='all', title=title, labeler=labeler)
